...
def parse_PC(response,meta):
    ...
    meta.N_watch = exeRe(response.text,getsee).replace("人",'')
    meta.W_watch = exeRe(response.text,getwant).replace("人",'')
    return meta
titleDict = dict()
import xlrd
book = xlrd.open_workbook("movie.xlsx")
...
# with open("aqy_drama_title.csv","r",encoding="gbk") as f:
#     a = f.readlines()[1:]
...
#             if name == name2:
#                 titleDict[ui] = name
#                 with open('url.csv','a+',encoding='utf-8') as m:
#                     m.write(name + ',' + ui + '\n')
#             else:
#                 with open("error.csv",'a+',encoding='utf-8') as s:
...
class TvmaospiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        c = response.text
        meta = metas()
        meta.ID = response.url.split("movie/")[1]
        meta.Title = titleDict.get(response.url,'')
        # zys = exeRe(c, zy, tp='')
        zys1 = exeRe(c,zy1,tp = '')
        meta.Role_info = get_data(zys1,zyp,"主演：")
        meta.Director = get_data(zys1,zyp,"导演：")
        ...
        meta.Screenwriter = get_data(zys1,zyp,'编剧：')
        meta.Location = get_data(zys1,zyp,'地区：')
        rx = exeRe(c,zy1,zyp,tp = '')##########
        meta.Intro_type = ''
        for ima in rx:
            if "类别" in ima:
                meta.Intro_type = exeRe(ima,getTPa,tp = '')

        meta.Intro_Era = get_data(zys1,zyp,"年份：")
        ...
        meta.Score = exeRe(c,scoreiunit) + exeRe(c,scoreidecimal)#################
        ...
        submainURL= response.url + "/chapter"
    ...
    def parse_RWGX(self,response):
        ...
        for i in ul:
            ...
            for l in li:
                p += exeRe(l,getP)
        meta.Relationship = p
        pcURL = "https://www.tvmao.com/movie/" + meta.get("ID")
    ...
    def parse_subMain(self,response):
        ...
        p2 = exeRe(c,pages3,getLI,tp='')
        for i in p2:
        # for i in newURL:
            newURL = exeRe(i, page4)
            meta.Sub_id = newURL.split('chapter/')[1].replace("\"",'')
            meta.Sub_info_num = meta['Sub_id']
            yield scrapy.Request(url="https://m.tvmao.com" + newURL, callback=self.parse_subMain2, meta=meta)
    ...
    def parse_subMain2(self,response):
        ...
        meta.Sub_info_title =exeRe(c,getH2)
        # meta.Sub_info_pic = json.loads(exeRe(c,mainimg) + ']')# 剧集图片 TODO 这个也没看到呀，是简介里的图片吗
        meta.Sub_info_pic = ''# 剧集图片 TODO 这个也没看到呀，是简介里的图片吗
        meta.Sub_info_detail_txt = exeRe(c,mainsumm2,mainp)
        meta.Sub_publishTime =exeRe(c,putime).replace("时间：",'')
